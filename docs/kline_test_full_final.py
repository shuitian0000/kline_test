...
# 文档完整
# 所有 Destiny / Access 模块全部补全，确保解压后 微信开发者工具即可直接运行完整小程序 demo，包括：
...
# kline-generator.js（K 线生成逻辑）

# explain-engine.js（Explain Engine，文本输出）

# explain-template.js（趋势、风险、策略模板）
...
# capability.js（付费层权限控制）

# Pages / Components / Docs 已经包含
# ✅ 完成效果
...
# Explain Engine 输出趋势、风险、策略、宏观波动
...
files = {
  ...
  "window": {"navigationBarTitleText": "人生K线"}
    ...
    # Pages / Components 同之前脚本（index & kline & kline-chart）略，保持一致

    # Destiny 完整模块
    f'{ROOT_DIR}/utils/destiny/lunar.js': """export function solarToLunar(solar){return solar;} // 简化示例
export function lunarToSolar(lunar){return lunar;}""",

    f'{ROOT_DIR}/utils/destiny/bazi.js': """import { lunarToSolar } from './lunar.js';
...
export function getBazi(birth){
  ...
  if(calendar==='lunar'){const solar=lunarToSolar(date); [year,month,day]=solar.split('-').map(Number);}
  else {[year,month,day]=date.split('-').map(Number);}
  const yTG=TIANGAN[(year-4)%10], yDZ=DIZHI[(year-4)%12];
  const mIndex=(year*12+month+12)%60, mTG=TIANGAN[mIndex%10], mDZ=DIZHI[mIndex%12];
  const baseDate=new Date(1900,0,31), curDate=new Date(year,month-1,day);
  const diff=Math.floor((curDate-baseDate)/86400000), dTG=TIANGAN[diff%10], dDZ=DIZHI[diff%12];
  const hIndex=Math.floor(hour/2), hTG=TIANGAN[(diff*12+hIndex)%10], hDZ=DIZHI[hIndex%12];
  const bazi={year:{tg:yTG,dz:yDZ},month:{tg:mTG,dz:mDZ},day:{tg:dTG,dz:dDZ},hour:{tg:hTG,dz:hDZ}};
  const wuxingBalance=getWuxingScore(bazi); return {bazi,wuxingBalance};
}
...
export function generateLifeKLine(profile){
  ...
  return stages.map(stage=>{
    ...
    lastClose=close
    return {...stage,open,close,high:Math.max(open,close)+adjustedVol,low:Math.min(open,close)-adjustedVol,trend:close>open?'up':close<open?'down':'flat',riskLevel,strategyBias,macroContext:macro,explainSeed:{age:stage.ageMid,volatility:adjustedVol,delta:close-open}}
  })
}""",
