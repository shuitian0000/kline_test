...
# 可以一次性生成整个 完整项目目录 + demo 小程序 + 社会周期模块 + Explain Engine + 文档，并压成 kline_test.zip，解压后即可直接打开微信开发者工具运行。
# 使用方法
# 1.将以上 Python 脚本保存为 generate_kline_demo.py
# 2.在 Python 环境下运行：
# python generate_kline_demo.py
# 3.当前目录会生成 kline_test.zip
# 4.解压后即可看到完整目录、社会周期模块、Explain Engine、K 线全屏交互、小程序 demo、文档
# 5.打开微信开发者工具即可运行
...
""",
  ...
  globalData: {
    ...
    profile: {
      name: '张三',
      gender: 'male',
      birth: { calendar:'solar', date:'1992-08-15', hour:10, location:'Hangzhou' },
      wuxingBalance: 0.02
    },
    kline: []
  },
  ...
  <kline-chart kline-data="{{kline}}" bind:select="onSelect" full-screen="{{fullScreen}}"/>
  ...
  onLoad() { this.setData({ kline: getApp().globalData.kline }) },
  ...
  properties: { klineData:Array, fullScreen:Boolean },
  data:{ candleRects:[] },
  lifetimes:{ ready(){ this.draw() } },
  observers:{ 'klineData,fullScreen':function(){ this.draw() } },
  methods:{
    draw() {
      ...
      this.properties.klineData.forEach((pt,i)=>{
        ...
        const color = pt.trend==='up'?'#4caf50': pt.trend==='down'?'#f44336':'#999'
...
print("✅ kline_test.zip 已生成！运行后可直接在微信开发者工具打开。")
